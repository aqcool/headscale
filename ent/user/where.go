...
// HasPreAuthKeys applies the HasEdge predicate on the "pre_auth_keys" edge.
func HasPreAuthKeys() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasPreAuthKeysWith applies the HasEdge predicate on the "pre_auth_keys" edge with a given conditions (other predicates).
func HasPreAuthKeysWith(preds ...predicate.PreAuthKey) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newPreAuthKeysStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
