...
type UserQuery struct {
	...
	withNodes       *NodeQuery
	withPreAuthKeys *PreAuthKeyQuery
...
func (_q *UserQuery) QueryNodes() *NodeQuery {
...
// QueryPreAuthKeys chains the current query on the "pre_auth_keys" edge.
func (_q *UserQuery) QueryPreAuthKeys() *PreAuthKeyQuery {
	query := (&PreAuthKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPreAuthKeys: _q.withPreAuthKeys.Clone(),
...
func (_q *UserQuery) WithNodes(opts ...func(*NodeQuery)) *UserQuery {
	query := (&NodeClient{config: _q.config}).Query()
...
// WithPreAuthKeys tells the query-builder to eager-load the nodes that are connected to
// the "pre_auth_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithPreAuthKeys(opts ...func(*PreAuthKeyQuery)) *UserQuery {
	query := (&PreAuthKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPreAuthKeys = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withPreAuthKeys; query != nil {
		if err := _q.loadPreAuthKeys(ctx, query, nodes,
			func(n *User) { n.Edges.PreAuthKeys = []*PreAuthKey{} },
			func(n *User, e *PreAuthKey) { n.Edges.PreAuthKeys = append(n.Edges.PreAuthKeys, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadNodes(ctx context.Context, query *NodeQuery, nodes []*User, init func(*User), assign func(*User, *Node)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(node.FieldUserID)
	}
...
func (_q *UserQuery) loadPreAuthKeys(ctx context.Context, query *PreAuthKeyQuery, nodes []*User, init func(*User), assign func(*User, *PreAuthKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.PreAuthKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PreAuthKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_pre_auth_keys
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_pre_auth_keys" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_pre_auth_keys" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
