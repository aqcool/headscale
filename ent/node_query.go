...
type NodeQuery struct {
	...
	withUser       *UserQuery
	withPreAuthKey *PreAuthKeyQuery
	withFKs        bool
...
func (_q *NodeQuery) QueryUser() *UserQuery {
...
// QueryPreAuthKey chains the current query on the "pre_auth_key" edge.
func (_q *NodeQuery) QueryPreAuthKey() *PreAuthKeyQuery {
...
func (_q *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withPreAuthKey: _q.withPreAuthKey.Clone(),
...
func (_q *NodeQuery) WithUser(opts ...func(*UserQuery)) *NodeQuery {
	query := (&UserClient{config: _q.config}).Query()
...
// WithPreAuthKey tells the query-builder to eager-load the nodes that are connected to
// the "pre_auth_key" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NodeQuery) WithPreAuthKey(opts ...func(*PreAuthKeyQuery)) *NodeQuery {
	query := (&PreAuthKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPreAuthKey = query
	return _q
}
...
func (_q *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	var (
		...
		loadedTypes = [2]bool{
			_q.withUser != nil,
			_q.withPreAuthKey != nil,
		}
	)
	if _q.withPreAuthKey != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Node, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPreAuthKey; query != nil {
		if err := _q.loadPreAuthKey(ctx, query, nodes, nil,
			func(n *Node, e *PreAuthKey) { n.Edges.PreAuthKey = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NodeQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Node, init func(*Node), assign func(*Node, *User)) error {
...
func (_q *NodeQuery) loadPreAuthKey(ctx context.Context, query *PreAuthKeyQuery, nodes []*Node, init func(*Node), assign func(*Node, *PreAuthKey)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].pre_auth_key_nodes == nil {
			continue
		}
		fk := *nodes[i].pre_auth_key_nodes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(preauthkey.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "pre_auth_key_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *NodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(node.FieldUserID)
		}
	}
