...
// UpdateUser updates a user using an updateFn.
func (hsdb *HSDatabase) UpdateUser(uid types.UserID, updateFn func(*types.User) error) (*types.User, error) {
	return Write(hsdb.DB, func(tx *gorm.DB) (*types.User, error) {
		return UpdateUser(tx, uid, updateFn)
	})
}

// UpdateUser applies a function to update a user in the database.
func UpdateUser(tx *gorm.DB, uid types.UserID, updateFn func(*types.User) error) (*types.User, error) {
	user, err := GetUserByID(tx, uid)
	if err != nil {
		return nil, err
	}
