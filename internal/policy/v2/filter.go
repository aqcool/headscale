...
func (pol *Policy) compileSSHPolicy(
	...
	for index, rule := range pol.SSHs {
		...
		var localpartByUser map[types.UserID]string
		if hasLocalpart {
		...
		appendRules := func(principals []*tailcfg.SSHPrincipal, uid types.UserID, hasUID bool) {
...
func resolveLocalparts(
	entries []SSHUser,
	users types.Users,
) map[types.UserID]string {
	if len(entries) == 0 {
		return nil
	}

	result := make(map[types.UserID]string)
...
func groupSourcesByUser(
...
) ([]types.UserID, map[types.UserID][]*tailcfg.SSHPrincipal, []*tailcfg.SSHPrincipal) {
	userIPSets := make(map[types.UserID]*netipx.IPSetBuilder)
	...
	principalsByUser := make(map[types.UserID][]*tailcfg.SSHPrincipal, len(userIPSets))
